...
Friend Class MoorSystem
	...
	Public Function InputML(ByVal FileNum As Short) As Boolean
		...
		Dim Payout, Scope, TopTen As Double
		Dim PayoutSur, PayoutOpr As Double
		Dim PretenSur, PretenOpr As Double
		Dim FLY, SprdAng, FLX, FLZ As Double
		Dim AnchorY, AnchorX, WaterDepth As Double
		Dim BtmSlp As String
		Dim AnchModel, AnchRemark As String
		Dim HoldCap As Double
		Dim SegTp As String
...
ErrorHandler: 
	...
	Public Sub MoorStiff(ByRef Stiff As Force, Optional ByRef ShipLoc As ShipGlobal = Nothing, Optional ByRef IsGlobal As Boolean = True)
		...
		Dim WaterDepth, Alpha As Double
		Dim dy, dx, Drz As Double
		Dim SgnY, SgnX, SgnR As Double
		Dim DFy, DFx, DMz As Double
		...
		If IsGlobal Then
			...
			DFx = Max(System.Math.Abs(-FGlob1.Fx + FGlob.Fx), 10#) * SgnX
		Else
			DFx = Max(System.Math.Abs(-FLocl1.Fx + FLocl.Fx), 10#) * SgnX
		End If
		...
		If IsGlobal Then
			Call FGlobFrmLocl(FGlob1, FLocl1, (ShipMove.Heading))
			DFy = Max(System.Math.Abs(-FGlob1.Fy + FGlob.Fy), 10#) * SgnY
		Else
			DFy = Max(System.Math.Abs(-FLocl1.Fy + FLocl.Fy), 10#) * SgnY
		End If
		...
		Call TotFMoor(FLocl1, ShipMove)
		DMz = Max(System.Math.Abs(-FLocl1.MYaw + FLocl.MYaw), 10#) * SgnR
	...
	Private Sub SLoclFrmGlob(ByRef SGlob As Force, ByRef SLocl As Force, ByRef Heading As Double)
