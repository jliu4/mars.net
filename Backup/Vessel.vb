...
Friend Class Vessel
	...
	Private msngShipDraftSur As Double
	Private msngShipDraftOpr As Double
	...
	Private Const DistTol As Double = 0.0001
	Private Const AngleTol As Double = 0.0001
	
	Private Const FreqS As Double = 0.02
	Private Const FreqE As Double = 2.4
	Private Const FreqD As Double = 0.02
	Private Const FreqE1 As Double = 0.2
	Private Const FreqE2 As Double = 1#
	Private Const FreqD1 As Double = 0.02
	Private Const FreqD2 As Double = 0.1
	...
	Public Property ShipDraftSur() As Double
		Get
	...
	Public Property ShipDraftOpr() As Double
		Get
	...
	Public ReadOnly Property ShipMass(ByVal Draft As Double) As Motion
		Get
			
			Dim N, i, Ns As Short
			Dim Rd As Double
			Dim My1, Mx1, Mm1 As Double
			Dim My2, Mx2, Mm2 As Double
	...
	Public ReadOnly Property ShipDamp(ByVal Draft As Double) As Motion
		Get
	...
	Public Function GetSigWFM(ByRef Cancelled As Boolean, ByRef frmProgress As System.Windows.Forms.Form, Optional ByVal Heading As Double = 0) As Motion
		
		Dim MotionRAO As Motion
		Dim Direct, Freq, Sw As Double
		Dim Sway, Surge, Yaw As Double
	...
	Public Function GetSigLFM(ByRef StiffLocl As Force, ByRef Cancelled As Boolean, ByRef frmProgress As System.Windows.Forms.Form, Optional ByRef ShipLoc As ShipGlobal = Nothing) As Motion
		...
		Dim Freq, Heading As Double
		Dim Freq0, FreqD0, Freq1 As Double
		Dim Sxy, Sxx, Sxz As Double
		Dim Sway, Surge, Yaw As Double
	...
	Public Function InputVsl(ByVal FileNum As Short) As Boolean
		...
		Dim DesY, DesX, DesH As Double
		Dim CurY, CurX, CurH As Double
		Dim OprD, SurD, CurD As Double
		Dim WD As Double
...
ErrorHandler: 
	...
	Private Function RAO(ByVal Draft As Double, ByVal Frequency As Double, ByVal Direction As Double) As Motion
	...
	Public Function InputRAOs(ByVal FileNum As Short) As Boolean
		...
		Dim NewRAOs As RAOs
		Dim Freq, Draft, Dummy As Double
		Dim RAOy, RAOx, RAOr As Double
		Dim msg As String
...
ErrorHandler: 
	...
	Private Function GetOriginalDamping(ByVal Draft As Double) As Motion
		Dim N, i, Ns As Short
		Dim Rd As Double
		Dim Dy1, Dx1, Dm1 As Double
		Dim Dy2, Dx2, Dm2 As Double
		Dim Damp As New Motion
...
ErrorHandler: 
	...
	Public Sub NextPosnVel(ByVal DT As Double, ByRef Acc() As Motion, ByRef Vel() As Motion, ByRef Pos() As ShipGlobal)
