...
Friend Class frmMove
	...
	Private Sub btnPayout_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles btnPayout.Click
		...
		Select Case Index
			...
			Case 1
				...
				TopTension = CDbl(CheckData(TmpStr)) * 1000#
	...
	Private Sub SaveLC()
		...
		With MoorLines
			...
			.WinchCap = CDbl(CheckData(txtConditions.Text,  , True)) * 1000# / FrcFactor
		End With
	...
	Public Sub CalcPayout(ByRef Index As Short, ByRef TargetLoc As ShipGlobal)
		...
		Select Case Index
			...
			Case 1
				...
				TopTension = CDbl(CheckData(TmpStr)) * 1000# / FrcFactor
	...
	Private Sub MoveVessel()
		...
		Dim OldHorFrc(MaxNumLines) As Double
		...
		Dim NewHorFrc(MaxNumLines) As Double
		...
		Dim A3n(3, MaxNumLines) As Double
		...
		Dim An3(MaxNumLines, 3) As Double
		...
		Dim Scope(MaxNumLines) As Double
		'UPGRADE_WARNING: Lower bound of array Payout was changed from 1 to 0. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="0F1C9BE1-AF9D-476E-83B1-17D43BECFF20"'
		Dim Payout(MaxNumLines) As Double
...
Again: 
	...
	Private Sub MoveVessel1(ByVal Pretension As Double)
		
		Dim i As Short
		Dim Payout, Scope As Double
